...
void Pneumaticarm3orderModel::computeAllModelDeriv(double& dt, const stateVec_t& X,const commandVec_t& U)
{
    fx = fxBase;
    fx(2,1) -= A21*cos(X(0));
    fx(2,0) += A21*sin(X(0))*X(1);
    fxx[0](2,1)+= A21*sin(X(0));
    fxx[0](2,0)+= A21*cos(X(0))*X(1);
